...
def test_own_keys_have_no_role_path_even_when_they_contain_a_slash():
    role_path, leaf = split_qualified_key("Irradiance [mW/cm2]")

    # A key containing a slash still splits, which is why the registry records
    # the role path rather than trying to re-derive it later.
    assert leaf == "cm2]"
    assert role_path == "Irradiance [mW"
